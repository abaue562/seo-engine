...
``taskq/celery_app.py``.  Only ONE beat process should run at a time.
...
Or via the Celery CLI:
    celery -A taskq.celery_app beat --loglevel=info
...
---------------------------------------------------
  ...
  run-rank-check       every 168 h → taskq.tasks.check_rankings   (weekly)
  run-content-decay    every 72 h  → taskq.tasks.scan_content_decay (3 days)
  run-learning-cycle   every 168 h → taskq.tasks.run_learning      (weekly)
...
# Ensure project root is on the path when running as `python -m taskq.beat`
